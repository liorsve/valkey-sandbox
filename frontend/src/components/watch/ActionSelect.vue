    ...
    <div class="action-select" :class="{ 'client-select': showClients }">
        ...
        <div class="select-container">
            <h2 class="select-title">
                {{ showClients ? 'Choose Your Glide Client' : 'What Would You Like to Watch?' }}
            </h2>

            <div class="options-grid">
                <template v-if="!showClients">
                    <button v-for="action in actions" :key="action.id" class="option-button"
                ...
                <template v-else>
                    <button v-for="client in glideClients" :key="client.id" class="option-button client-button"
    ...
    setup( _, { emit } ) {
        ...
        const glideClients = [
            {
                ...
                id: 'glide-node',
                description: 'Blazing fast and non-blocking, perfect for Node.js ninjas',
                icon: '🔋'
            },
            {
        ...
        const selectClient = ( client ) => {
            try {
                ...
                const language = client.id.includes( 'python' ) ? 'python' :
                    client.id.includes( 'java' ) ? 'java' : 'javascript';
